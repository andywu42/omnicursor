...
class TestRuffDiagnostics:
    def test_python_file_triggers_ruff_check(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """Original stub: Python file causes ruff to be invoked."""
        calls: List[Any] = []
        monkeypatch.setattr(
            _file_edit.subprocess,
            "run",
            lambda cmd, **kw: calls.append(cmd) or _FakeRunResult(),
        )
    ...
    def test_ruff_called_with_check_subcommand(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        calls: List[Any] = []
        monkeypatch.setattr(
    ...
    def test_ruff_prefers_repo_venv_binary(
    ...
    def test_ruff_findings_zero_on_clean_file(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.setattr(
            _file_edit.subprocess,
            "run",
            lambda cmd, **kw: _FakeRunResult(stdout="", returncode=0),
        )
        result = _mod.run_ruff_check("foo.py")
        assert result == 0
    ...
    def test_ruff_findings_reflected_in_handle_edit(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.setattr(
            _file_edit.subprocess,
    ...
    def test_non_python_ruff_findings_zero(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
...
class TestTscDiagnostics:
    def test_typescript_file_triggers_tsc_check(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        calls: List[Any] = []
        monkeypatch.setattr(
    ...
    def test_non_typescript_file_skips_tsc(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        calls: List[Any] = []
        monkeypatch.setattr(
    ...
    def test_tsc_findings_counted(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr(
            _file_edit.subprocess,
            "run",
            lambda cmd, **kw: _FakeRunResult(stdout=output, returncode=1),
        )
        result = _mod.run_tsc_check("foo.ts")
        assert result == 2

    def test_tsc_findings_zero_on_clean_file(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.setattr(
            _file_edit.subprocess,
    ...
    def test_tsc_timeout_returns_zero(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr(
            ...
            "run",
            lambda cmd, **kw: (_ for _ in ()).throw(
                subprocess.TimeoutExpired("tsc", 15)
            ),
        )
        result = _mod.run_tsc_check("foo.ts")
        assert result == 0

    def test_tsc_findings_reflected_in_handle_edit(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
    ...
    def test_non_typescript_tsc_findings_zero(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        calls: List[Any] = []
        monkeypatch.setattr(
...
class TestHandleEdit:
    ...
    def test_conversation_id_preserved(self) -> None:
        result = _mod.handle_edit(
            {"file_path": "a.py", "edits": [], "conversation_id": "c-xyz"}
        )
...
class TestCorrelationThreading:
    def _run(
        ...
        monkeypatch.setattr(
            _mod,
            "read_stdin",
            lambda: {"file_path": file_path, "edits": [], "conversation_id": "c-001"},
        )
        monkeypatch.setattr(sys, "stdout", io.StringIO())
        _mod.main()
        return events[0]

    def test_correlation_id_read_from_session(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
    ...
    def test_correlation_id_on_python_file(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.setattr(
            _file_edit.subprocess, "run", lambda cmd, **kw: _FakeRunResult()
        )
        e = self._run(
            monkeypatch,
            file_path="main.py",
            session={"latest_correlation_id": "py000001abc1"},
        )
        assert e["correlation_id"] == "py000001abc1"

    def test_correlation_id_on_typescript_file(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        e = self._run(
            monkeypatch,
            file_path="app.ts",
            session={"latest_correlation_id": "ts000001abc1"},
        )
...
class TestTypedEventSchema:
    def _run(
        ...
        monkeypatch.setattr(
            _mod,
            "read_stdin",
            lambda: {"file_path": file_path, "edits": edits, "conversation_id": conv},
        )
        monkeypatch.setattr(sys, "stdout", io.StringIO())
        _mod.main()
        return events[0]
    ...
    def test_tsc_findings_zero_for_non_typescript(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
    ...
    def test_stdout_is_empty_dict(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr(
            _mod, "read_stdin", lambda: {"file_path": "a.md", "edits": []}
        )
        buf = io.StringIO()
    ...
    def test_event_logged_to_events_jsonl(
        ...
        monkeypatch.setattr(
            _mod,
            "read_stdin",
            lambda: {"file_path": "x.md", "edits": [], "conversation_id": "t-1"},
        )
        monkeypatch.setattr(sys, "stdout", io.StringIO())
        _mod.main()
        assert len(events) == 1
    ...
    def test_ruff_findings_zero_for_non_python(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
